...
describe('Members export -> import round-trip', function () {
  ...
  async function seedMembers(prefix, labelName) {
    ...
    const stripeMember = await models.Member.add({
      email_disabled: false,
      email: emails.stripe,
      name: 'Stripe Member',
      labels: [label],
    ...
    const customer = await models.MemberStripeCustomer.add({
      member_id: stripeMember.id,
      customer_id: `cus_${prefix}`,
      name: 'Stripe Member',
      email: emails.stripe,
    });
    const subscription = await models.StripeCustomerSubscription.add({
      subscription_id: `sub_${prefix}`,
      customer_id: customer.get('customer_id'),
      stripe_price_id: `price_${prefix}`,
      status: 'active',
      cancel_at_period_end: false,
      current_period_end: '2032-05-19 09:08:53',
      start_date: '2020-05-19 09:08:53',
      plan_id: `price_${prefix}`,
      plan_nickname: 'Yearly',
      plan_interval: 'year',
      plan_amount: 5000,
      plan_currency: 'USD',
    });
    await models.Base.knex('members_current_subscription').insert({
      member_id: stripeMember.id,
      subscription_id: subscription.get('id'),
    });

    const giftMember = await models.Member.add({
      email_disabled: false,
      email: emails.gift,
      name: 'Gift Member',
      status: 'gift',
    ...
    await models.Gift.add({
      token: `${prefix}-gift-token`,
      buyer_email: 'buyer@example.com',
      buyer_member_id: null,
      redeemer_member_id: giftMember.id,
      tier_id: tier.id,
      cadence: 'year',
      duration: 1,
      currency: 'usd',
      amount: 5000,
      stripe_checkout_session_id: `cs_${prefix}`,
      stripe_payment_intent_id: `pi_${prefix}`,
  ...
  async function assertSurvivesWithCoreFields(emails) {
    ...
    assert.ok(
      core.labels.map((l) => l.name).some((n) => n.startsWith('Round-Trip')),
      'labels round-trip',
    );
  }
  ...
  beforeAll(async function () {
    ...
    await models.Product.add({
      name: 'Round-Trip Tier',
      slug: 'round-trip-tier',
      type: 'paid',
      active: true,
      visibility: 'public',
    });
  ...
  it('exports a custom field and re-imports its value onto a fresh member', async function () {
    ...
    await models.Member.add({
      ...
      note: 'cf note',
      newsletters: [{ id: newsletter.id }],
      labels: [{ name: customLabel.get('name') }],
    });
  ...
  it('round-trips an address custom field and leaves a value-less member untouched', async function () {
    ...
    const addressLabel = (await models.Label.findAll()).models.find(
      (l) => l.get('name') === 'Round-Trip Address',
    );
    ...
    await models.Member.add({
      ...
      name: 'Plain Member',
      newsletters: [{ id: newsletter.id }],
      labels: [{ name: addressLabel.get('name') }],
    });
