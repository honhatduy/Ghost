...
describe('RouterController', function () {
  ...
  describe('createCheckoutSession', function () {
    ...
    it('sets ghostSignupContext to has_precheckout_magic_link when checkout creates a signup magic link', async function () {
      const magicLinkService = {
        getMagicLink: sinon
          .stub()
          .resolves('https://example.com/members/?token=abc123&action=signup'),
      };
    ...
    describe('_getSubscriptionCheckoutData', function () {
      ...
      it('returns a BadRequestError if both offerId and tierId are provided', async function () {
        ...
        try {
          await routerController._getSubscriptionCheckoutData({
            tierId: 'tier_123',
            offerId: 'offer_123',
          });
          assert.fail('Expected function to throw BadRequestError');
      ...
      it('returns a BadRequestError if tierId is provided wihout a cadence', async function () {
        ...
        try {
          ...
          assert.equal(
            error.context,
            'Expected cadence to be "month" or "year", received undefined',
          );
        }
      });
      ...
      it('returns a BadRequestError if offer is not active', async function () {
        offersAPI = {
          getOffer: sinon
            .stub()
            .resolves({ id: 'archived_offer_123', duration: 'forever', status: 'archived' }),
        };
    ...
    describe('gift checkout', function () {
      ...
      it('maps gift checkout requests to the module interface', async function () {
        ...
        sinon.assert.calledWith(
          giftService.service.startCheckout,
          sinon.match({
            tierId: 'tier_123',
            cadence: 'month',
            duration: undefined,
      ...
      it('rejects when offerId is provided', async function () {
        giftService.service.startCheckout.rejects(
  ...
  describe('createCheckoutSetupSession return URL validation', function () {
    ...
    it('passes same-origin successUrl and cancelUrl through unchanged', async function () {
      ...
      await controller.createCheckoutSetupSession(
        mockReq({
  ...
  describe('sendMagicLink', function () {
    describe('newsletters', function () {
      ...
      it('adds specified newsletters to the tokenData', async function () {
        ...
        const newsletterNamesFilter = newsletterNames.map(
      ...
      it('validates specified newsletters', async function () {
        ...
        req.body.newsletters = [{ name: INVALID_NEWSLETTER_NAME }];
    ...
    describe('gift token forwarding', function () {
      ...
      it('forwards giftToken for signup and subscribe flows', async function () {
        ...
        sinon.assert.calledOnce(sendEmailWithMagicLinkStub);
        assert.equal(
          sendEmailWithMagicLinkStub.firstCall.args[0].tokenData.giftToken,
          'gift-token-123',
        );
      });
  ...
  describe('verifyOTC', function () {
    ...
    describe('input validation', function () {
      it('should throw BadRequestError when otc is missing', async function () {
        ...
        await assert.rejects(routerController.verifyOTC(req, res), {
      ...
      it('should throw BadRequestError for invalid otc formats', async function () {
        ...
        for (const invalid of invalidOtcs) {
          ...
          await assert.rejects(routerController.verifyOTC(req, res), { code: 'INVALID_OTC' });
        }
      });
    ...
    describe('token provider integration', function () {
      ...
      it('should throw BadRequestError when tokenProvider is missing', async function () {
        ...
        await assert.rejects(routerController.verifyOTC(req, res), { code: 'OTC_NOT_SUPPORTED' });
      });
      ...
      it('should handle tokenProvider.getTokenByRef throwing error', async function () {
        ...
        await assert.rejects(routerController.verifyOTC(req, res), Error);
      });
    ...
    function createRouterController(overrides = {}) {
      return new RouterController({
        ...
        memberRepository: {
          get: sinon
            .stub()
            .resolves(createMockMember(overrides.subscriptions || createMockSubscription())),
        },
        tokenService,
