...
test.describe('Ghost Admin - Custom field filter columns', () => {
  ...
  test('a composite field reads as the same one line in the column and on the member', async ({
    page,
  }) => {
    test.slow();
    ...
    const member = await memberFactory.create({
      name: memberName,
      email: `shipped-${stamp}@example.com`,
    });
    ...
    await expect(memberDetailsPage.customFieldRow(fieldName)).toHaveAccessibleName(
      `Edit ${fieldName}: ${expectedLine}`,
    );
